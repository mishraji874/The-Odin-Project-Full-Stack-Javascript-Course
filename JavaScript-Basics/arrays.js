...
// Instead you can use for..of loop to compare arrays item-by-it
...
const result = sumOfTripledEvens(numbers);
console.log(result); // Output: 36 (2*3 + 4*3 + 6*3 = 6 + 12 + 18 = 36)
...
const arr = [1, 2, 3, 4, 5];
const mappedArr1 = arr.map(addOne);
...
// Assignment - Rewrite the sumOfTripledEvens(array) function using these three methods.

function sumOfTripledEvens(array) {
  return array
    .filter(num => num % 2 === 0) // Step 1: Filter even numbers
    .map(num => num * 3) // Step 2: Triple each even number
    .reduce((sum, current) => sum + current, 0); // Step 3: Sum all tripled numbers
}

// Example usage:
const number = [1, 2, 3, 4, 5, 6];
const ans = sumOfTripledEvens(numbers);
console.log(ans); // Output: 36 (2*3 + 4*3 + 6*3 = 6 + 12 + 18 = 36)